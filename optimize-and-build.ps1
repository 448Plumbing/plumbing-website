...
Step "SEO enhance" {
  & (Join-Path $scriptDir 'seo-enhance.ps1') -Root $SeoRoot -BaseUrl $BaseUrl -DryRun:$false | Write-Host
}

# 2) Audit and fix (GitHub-friendly output when -FailOnError)
Step "Site audit & fix" {
  $auditArgs = @('-Root', $Root, '-Fix')
  if ($FailOnError) { $auditArgs += @('-FailOnError') }
  & (Join-Path $scriptDir 'site-audit.ps1') @auditArgs | Write-Host
}

# 3) Build to dist
Step "Build" {
  & (Join-Path $scriptDir 'build.ps1') -Root $Root -Dist $Dist -BaseUrl $BaseUrl -DryRun:$false | Write-Host
}
